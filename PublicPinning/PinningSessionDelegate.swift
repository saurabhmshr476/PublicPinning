...
//  PublicPinning
//
//  Created by Saurabh Mishra on 29/06/20.
//  Copyright © 2020 Saurabh Mishra. All rights reserved.
//

import Alamofire

class PinningSessionDelegate: SessionDelegate {
    ...
    override init() {
        ...
        sessionDidReceiveChallengeWithCompletion = { session, challenge, completion in
            ...
            if let serverCertificate = SecTrustGetCertificateAtIndex(trust, 0), let serverCertificateKey = PinningSessionDelegate.publicKey(for: serverCertificate) {
                if PinningSessionDelegate.pinnedKeys().contains(serverCertificateKey) {
                    completion(.useCredential, URLCredential(trust: trust))
                    return
                }
            }
    ...
    private static func pinnedCertificates() -> [Data] {
        ...
        if let pinnedCertificateURL = Bundle.main.url(forResource: "sureify", withExtension: "cer") {
            do {
